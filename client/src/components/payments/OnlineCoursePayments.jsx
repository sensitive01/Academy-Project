...
const OnlineCoursePayments = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      center: true,
      cell: row => (
        <span className={`px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider ${row.status === "success" || row.status === "paid" ? "bg-green-100 text-green-700" :
    ...
    },
    {
      ...
      cell: row => (
        <div className="flex items-center gap-1 justify-center">
          ...
          <button onClick={() => setConfirmModal({ isOpen: true, id: row._id })} className="text-red-500 hover:text-red-700 hover:bg-red-50 p-2 rounded-lg transition-colors" title="Delete Payment">
  ...
  return (
    <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-4 sm:p-6 overflow-hidden">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-bold text-slate-800">Online Course Payments</h2>
      ...
      <CustomDataTable
        ...
        additionalHeaderContent={
          <div className="flex items-center gap-2 flex-nowrap overflow-x-auto py-1">
            ...
            {(selectedCourse !== "all" || fromDate !== "" || toDate !== "") && (
              <button
                onClick={() => {
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-[10000] p-4" onClick={() => setConfirmModal({ isOpen: false, id: null })}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
            <div className="p-6">
              ...
              <h3 className="text-lg font-bold text-slate-800 mb-2">Delete Payment</h3>
              <p className="text-slate-600 text-sm">Are you sure you want to delete this payment? This action cannot be undone.</p>
            </div>
