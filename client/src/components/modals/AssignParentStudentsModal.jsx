...
const AssignParentStudentsModal = ({ parent, onClose, onAssignSuccess }) => {
  ...
  // We need to fetch the parent's current children first to initialize selectedStudentIds
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Filter out students who are already connected to a DIFFERENT parent
        const validStudents = studentsList.filter(st => {
          if (!st.parent) return true;
          const pId = typeof st.parent === 'object' ? st.parent._id : st.parent;
          return pId === parent._id;
        });
        
        setStudents(validStudents);

        // Fetch current children for this parent to pre-select them
        if (parent && parent._id) {
          const childrenRes = await api.get(`/parent/parent/${parent._id}`);
          if (childrenRes.data) {
            setSelectedStudentIds(childrenRes.data.map(child => child._id));
          }
        }
      } catch (error) {
  ...
  return (
    <div className="fixed inset-0 z-[60] overflow-y-auto bg-slate-900/50 backdrop-blur-sm flex items-start justify-center p-4 py-10">
        ...
        <div className="flex items-center justify-between mb-8 border-b border-gray-100 pb-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-brand-600 font-semibold mt-1">Parent: {parent.name} ({parent.email})</p>
            </div>
          </div>
          ...
          <div className="relative mb-8" ref={dropdownRef}>
            ...
            <div className="relative">
              ...
              <input 
                type="text" 
                autoComplete="off"
                placeholder={loading ? "Loading students..." : "Type a name, ID, or email to search..."} 
                  ...
                  filteredDropdown.map(st => (
                    ...
                    >
                      <div>
                        ...
                        <div className="text-xs text-gray-500 mt-0.5">{st.email}</div>
                      </div>
          ...
          <div className="mb-8">
            ...
            {selectedStudentIds.length === 0 ? (
              <div className="p-8 bg-gray-50 border border-dashed border-gray-300 rounded-2xl text-center flex flex-col items-center justify-center">
                ...
                <p className="text-xs text-gray-400 mt-1">Search and click to assign students to this parent.</p>
              </div>
            ) : (
                  ...
                  return (
                    <div key={id} className="group flex items-center justify-between p-3.5 bg-white border border-gray-200 rounded-xl shadow-sm hover:border-brand-300 hover:shadow-md transition-all">
                      <div className="flex flex-col overflow-hidden pr-3">
                        ...
                        <span className="text-xs text-gray-500 font-medium mt-0.5">{st.email}</span>
                      </div>
                      <button 
                        type="button" 
...
export default AssignParentStudentsModal;
