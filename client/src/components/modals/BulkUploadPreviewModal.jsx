...
const BulkUploadPreviewModal = ({ data, onClose, onSave, onPreviewRow }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await onSave({ data: previewData, template: selectedTemplate });
    } finally {
          ...
          <table className="w-full text-sm text-left">
            <thead className="text-xs text-slate-600 uppercase bg-slate-100 sticky top-0 z-10">
              <tr>
                ...
                <th className="px-4 py-3 whitespace-nowrap text-center text-slate-600 bg-slate-100 sticky right-0 z-20 border-l border-slate-200">Actions</th>
              </tr>
            </thead>
            <tbody>
              {previewData.map((row, rIndex) => (
                <tr key={rIndex} className="bg-white border-b border-slate-100 hover:bg-slate-50">
                  <td className="px-4 py-3 font-medium text-slate-900">{rIndex + 1}</td>
                  {headers.map(h => (
                    <td key={h} className="px-4 py-3">
                      <input
                        type="text"
                        value={row[h] !== undefined && row[h] !== null ? row[h] : ''}
                        onChange={(e) => handleEditCell(rIndex, h, e.target.value)}
                        disabled={isSubmitting}
                        className="w-full bg-transparent border-b border-transparent hover:border-slate-300 focus:border-brand-500 focus:ring-0 px-1 py-1 transition-colors min-w-[80px] disabled:opacity-50"
                      />
                    </td>
                  ))}
                  <td className="px-4 py-3 text-center sticky right-0 z-10 bg-white border-l border-slate-100 group-hover:bg-slate-50 flex items-center justify-center gap-2">
                    {onPreviewRow && (
                    ...
                    <button disabled={isSubmitting} onClick={() => handleDeleteRow(rIndex)} className="p-1.5 text-red-500 hover:bg-red-50 rounded-lg transition-colors disabled:opacity-50" title="Delete Row">
                      <Trash2 size={16} />
                    </button>
                  </td>
                </tr>
        ...
        <div className="flex gap-3 pt-4 mt-4 border-t border-slate-100 shrink-0">
          ...
          >
            {isSubmitting ? (
              ...
              <><Save size={18} /> Confirm Upload ({previewData.length} Records)</>
            )}
          </button>
        </div>
      </div>
